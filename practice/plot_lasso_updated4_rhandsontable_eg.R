  ...
  ui <- fluidPage(
    sidebarLayout(position='right',
                  ...
                  mainPanel(
                    ...
                    fluidRow(
                      column(2,
                             selectInput('data.src','Choose Dataset',
                                         choices=c('elements','principal components'),selected='elements'),
                      ...
                      column(3,offset=0.5,
                             ...
                             sliderInput('int.set','Set Confidence Interval',
  ...
  server <- function(input, output) {
    ...
    output$sel <- renderUI({
      checkboxGroupInput("groups", "Groups to show:",
                         unique(data.sel()[,3]),
                         selected=unique(data.sel()[which(unique(data.sel())[,3] != ''),3]))
    })
